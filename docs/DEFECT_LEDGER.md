...
## [BUG-037] - Date: 2026-08-21 - OPEN (fix in progress)
...
* **Resolution:** Pending -- see `spec.md` (this cycle) for the remediation plan: an approval-evidence requirement between the spec and design checkpoints, plus a per-cycle archive of `spec.md`/`design.md` content so checkpoint history is queryable without manual git digging.
* **Files Touched:** `docs/DEFECT_LEDGER.md` (this entry); `scripts/check_phase_gate.py` and related files pending the fix commit.
