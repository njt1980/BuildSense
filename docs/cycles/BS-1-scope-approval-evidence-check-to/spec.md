...
1. A plain `git push` does **not** transfer notes refs â€” confirmed via a
...
2. Even after an *explicit* `git push origin refs/notes/approvals`, a fresh
   `git clone` does **not** fetch notes by default â€” confirmed via the same
...
CI run** that touches `design.md` or a source file â€” regardless of whether
...
notes into CI (that was considered and explicitly declined â€” it would make
...
- `.githooks/pre-commit`'s existing invocation (`python
  scripts/check_phase_gate.py`, no flags) is **unchanged** â€” the local hook
...
- A short comment near `check_approval_evidence()` (or immediately above
  ...
  *why* â€” future readers should not "fix" this by trying to push notes into
...
- `scripts/tests/test_check_phase_gate.py` gets a new test: a `design.md`
  ...
  covered by `BS-001`'s existing tests â€” this new test only needs to prove
  the flag's effect).
...
- No attempt to add note-author identity or cryptographic verification â€”
...
- No changes to `AGENTS.md`'s Phase 1/2 approval-note instructions â€” the
